...
export async function getPreviousRounds(limit = 10): Promise<DailyRoundSummary[]> {
  try {
    ...
    const dates = await kv.lrange('round_history_dates', 0, limit * 2) // Fetch more to account for duplicates
    if (!dates || dates.length === 0) return []

    // Remove duplicate dates (keep first occurrence = most recent)
    const uniqueDates = Array.from(new Set(dates)).slice(0, limit)

    const keys = uniqueDates.map(d => `round_summary:${d}`)
