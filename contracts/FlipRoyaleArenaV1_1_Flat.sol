...
library Math {
     ...
     *
     * The result is stored in two 256 variables such that sum = high * 2T-‘¸¡‘¸¬ + low.
     */
     ...
     *
     * The result is stored in two 256 variables such that product = high * 2T-‘¸¡‘¸¬ + low.
     */
    function mul512(uint256 a, uint256 b) internal pure returns (uint256 high, uint256 low) {
        // 512-bit multiply [high low] = x * y. Compute the product mod 2T-‘¸¡‘¸¬ and mod 2T-‘¸¡‘¸¬ - 1, then use
        // the Chinese Remainder Theorem to reconstruct the 512 bit result. The result is stored in two 256
        // variables such that product = high * 2T-‘¸¡‘¸¬ + low.
        assembly ("memory-safe") {
    ...
    /**
     * @dev Unsigned saturating addition, bounds to `2T-‘¸¡‘¸¬ - 1` instead of overflowing.
     */
    ...
    /**
     * @dev Unsigned saturating multiplication, bounds to `2T-‘¸¡‘¸¬ - 1` instead of overflowing.
     */
    ...
    function mulDiv(uint256 x, uint256 y, uint256 denominator) internal pure returns (uint256 result) {
        unchecked {
            ...
            // Make sure the result is less than 2T-‘¸¡‘¸¬. Also prevents denominator == 0.
            if (denominator <= high) {
            ...
            assembly ("memory-safe") {
                ...
                // Flip twos such that it is 2T-‘¸¡‘¸¬ / twos. If twos is zero, then it becomes one.
            ...
            // Invert denominator mod 2T-‘¸¡‘¸¬. Now that denominator is an odd number, it has an inverse modulo 2T-‘¸¡‘¸¬ such
            // that denominator * inv ‘ÎÌ 1 mod 2T-‘¸¡‘¸¬. Compute the inverse by starting with a seed that is correct for
            // four bits. That is, denominator * inv ‘ÎÌ 1 mod 2‘¸+.
            uint256 inverse = (3 * denominator) ^ 2;

            // Use the Newton-Raphson iteration to improve the precision. Thanks to Hensel's lifting lemma, this also
            // works in modular arithmetic, doubling the correct bits in each step.
            inverse *= 2 - denominator * inverse; // inverse mod 2‘¸©
            inverse *= 2 - denominator * inverse; // inverse mod 2T¶‘¸¬
            inverse *= 2 - denominator * inverse; // inverse mod 2T-T-
            inverse *= 2 - denominator * inverse; // inverse mod 2‘¸¬‘¸+
            inverse *= 2 - denominator * inverse; // inverse mod 2T¶T-‘¸©
            inverse *= 2 - denominator * inverse; // inverse mod 2T-‘¸¡‘¸¬

            // Because the division is now exact we can divide by multiplying with the modular inverse of denominator.
            // This will give us the correct result modulo 2T-‘¸¡‘¸¬. Since the preconditions guarantee that the outcome is
            // less than 2T-‘¸¡‘¸¬, this is the final result. We don't need to compute the high bits of the result and high
    ...
    function invMod(uint256 a, uint256 n) internal pure returns (uint256) {
        unchecked {
            ...
            // ax + ny = 1
            // ax = 1 + (-y)n
            // ax ‘ÎÌ 1 (mod n) # x is the inverse of a modulo n
     ...
     * prime, then `a**(p-1) ‘ÎÌ 1 mod p`. As a consequence, we have `a * a**(p-2) ‘ÎÌ 1 mod p`, which means that
    ...
    function sqrt(uint256 a) internal pure returns (uint256) {
        unchecked {
            ...
            // In this function, we use Newton's method to get a root of `f(x) := xT- - a`. It involves building a
            // sequence x_n that converges toward sqrt(a). For each iteration x_n, we also define the error between
            // the current value as `+¡_n = | x_n - sqrt(a) |`.
            //
            // For our first estimation, we consider `e` the smallest power of 2 which is bigger than the square root
            // of the target. (i.e. `2**(e-1) ‘ÎÒ sqrt(a) < 2**e`). We know that `e ‘ÎÒ 128` because `(2T¶T-‘¸©)T- = 2T-‘¸¡‘¸¬` is
            // bigger than any uint256.
            //
            // By noticing that
            // `2**(e-1) ‘ÎÒ sqrt(a) < 2**e ‘Â∆ (2**(e-1))T- ‘ÎÒ a < (2**e)T- ‘Â∆ 2**(2*e-2) ‘ÎÒ a < 2**(2*e)`
            ...
            // We now have x_n such that `x_n = 2**(e-1) ‘ÎÒ sqrt(a) < 2**e = 2 * x_n`. This implies +¡_n ‘ÎÒ 2**(e-1).
            //
            // We can refine our estimation by noticing that the middle of that interval minimizes the error.
            // If we move x_n to equal 2**(e-1) + 2**(e-2), then we reduce the error to +¡_n ‘ÎÒ 2**(e-2).
            // This is going to be our x_0 (and +¡_0)
            xn = (3 * xn) >> 1; // +¡_0 := | x_0 - sqrt(a) | ‘ÎÒ 2**(e-2)

            // From here, Newton's method give us:
            // x_{n+1} = (x_n + a / x_n) / 2
            //
            // One should note that:
            // x_{n+1}T- - a = ((x_n + a / x_n) / 2)T- - a
            //              = ((x_nT- + a) / (2 * x_n))T- - a
            //              = (x_n‘¸+ + 2 * a * x_nT- + aT-) / (4 * x_nT-) - a
            //              = (x_n‘¸+ + 2 * a * x_nT- + aT- - 4 * a * x_nT-) / (4 * x_nT-)
            //              = (x_n‘¸+ - 2 * a * x_nT- + aT-) / (4 * x_nT-)
            //              = (x_nT- - a)T- / (2 * x_n)T-
            //              = ((x_nT- - a) / (2 * x_n))T-
            //              ‘Î— 0
            // Which proves that for all n ‘Î— 1, sqrt(a) ‘ÎÒ x_n
            //
            // This gives us the proof of quadratic convergence of the sequence:
            // +¡_{n+1} = | x_{n+1} - sqrt(a) |
            //         = | (x_n + a / x_n) / 2 - sqrt(a) |
            //         = | (x_nT- + a - 2*x_n*sqrt(a)) / (2 * x_n) |
            //         = | (x_n - sqrt(a))T- / (2 * x_n) |
            //         = | +¡_nT- / (2 * x_n) |
            //         = +¡_nT- / | (2 * x_n) |
            //
            ...
            // +¡_1 = +¡_0T- / | (2 * x_0) |
            //     ‘ÎÒ (2**(e-2))T- / (2 * (2**(e-1) + 2**(e-2)))
            //     ‘ÎÒ 2**(2*e-4) / (3 * 2**(e-1))
            //     ‘ÎÒ 2**(e-3) / 3
            //     ‘ÎÒ 2**(e-3-log2(3))
            //     ‘ÎÒ 2**(e-4.5)
            //
            // For the following iterations, we use the fact that, 2**(e-1) ‘ÎÒ sqrt(a) ‘ÎÒ x_n:
            // +¡_{n+1} = +¡_nT- / | (2 * x_n) |
            //         ‘ÎÒ (2**(e-k))T- / (2 * 2**(e-1))
            //         ‘ÎÒ 2**(2*e-2*k) / 2**e
            //         ‘ÎÒ 2**(e-2*k)
            xn = (xn + a / xn) >> 1; // +¡_1 := | x_1 - sqrt(a) | ‘ÎÒ 2**(e-4.5)  -- special case, see above
            xn = (xn + a / xn) >> 1; // +¡_2 := | x_2 - sqrt(a) | ‘ÎÒ 2**(e-9)    -- general case with k = 4.5
            xn = (xn + a / xn) >> 1; // +¡_3 := | x_3 - sqrt(a) | ‘ÎÒ 2**(e-18)   -- general case with k = 9
            xn = (xn + a / xn) >> 1; // +¡_4 := | x_4 - sqrt(a) | ‘ÎÒ 2**(e-36)   -- general case with k = 18
            xn = (xn + a / xn) >> 1; // +¡_5 := | x_5 - sqrt(a) | ‘ÎÒ 2**(e-72)   -- general case with k = 36
            xn = (xn + a / xn) >> 1; // +¡_6 := | x_6 - sqrt(a) | ‘ÎÒ 2**(e-144)  -- general case with k = 72

            // Because e ‘ÎÒ 128 (as discussed during the first estimation phase), we know have reached a precision
            // +¡_6 ‘ÎÒ 2**(e-144) < 1. Given we're operating on integers, then we can ensure that xn is now either
            // sqrt(a) or sqrt(a) + 1.
...
library ECDSA {
    ...
    function tryRecover(
        ...
        // the valid range for s in (301): 0 < s < secp256k1n +¿ 2 + 1, and for v in (302): v ‘ÍÍ {27, 28}. Most
...
// File contracts/FlipRoyaleArenaV1_1.sol
...
contract FlipRoyaleArenaV1_1 is Ownable, ReentrancyGuard, Pausable {
    ...
    // ‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…‘Ú…
